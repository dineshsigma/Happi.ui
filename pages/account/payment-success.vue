    ...
    <section class='ps-page--my-account'>
        ...
        <div class='ps-my-account'>
            <div class='container' style='text-align:center;'>
                    ...
                    <defs>
                        <style>
                            ...
                            #squiggle-mask-path {
                                ...
                                animation: animate-stroke 1s 1.1s forwards cubic-bezier(0, 1, 0.53, 1);
                            }

                            #left-line-mask-path {
                                stroke-dasharray: 38;
                                stroke-dashoffset: -38;
                                animation: animate-stroke 1s 1.2s forwards cubic-bezier(0, 1, 0.53, 1);
                            }
                            ...
                            #blue-star {
                                ...
                                animation: move-stars 1s 1025ms forwards cubic-bezier(0, 1, 0.53, 1);
                                display: inherit;
                            }

                            #yellow-star {
                                transform: translate(-10px, 45px);
                                animation: move-stars 1s 1050ms forwards cubic-bezier(0, 1, 0.53, 1);
                            }
                            ...
                            .dot {
                                animation: reveal-dots 0.9s 1.2s both cubic-bezier(0, 1, 0.53, 1);
                                opacity: 0;
                ...
                <h4 v-if='trasnactionDetails != null'>Order Id: {{ trasnactionDetails.orderId }}</h4>
                <h4 v-if='trasnactionDetails != null'>Payment Id:
                    {{ trasnactionDetails.paymentId }}</h4>


                <div class='col-xl-12 col-xl-offset-3 col-lg-12 col-md-12 col-sm-12 col-12' v-for="orderDetails in ordersDetails">
                        ...
                        <div class='ps-section__content'>
                            <div class='row'>
                                ...
                                <div class='col-md-12 col-12'>
                                    <figure class='ps-block--invoice address'
                                            v-if="orderDetails.deliveryType === 'pickup'">
                                        <figcaption>
                                            Pickup Date : {{ orderDetails.selectedStore.timeSlot }}
                                        </figcaption>
                                        <div class='ps-block__content'>
                                            <strong>
                                            ...
                                            <p>
                                                Address:
                                                {{ orderDetails.selectedStore.FullPostalAddress }}{{ orderDetails.selectedStore.Pincode }}
                                            </p>
                                            <p>
                                                Phone:
                                                {{ orderDetails.selectedStore.MobileNumber }}

                                            </p>
                                        </div>
                                    </figure>
                                    <figure class='ps-block--invoice address' v-else>
                                        <figcaption>
                                            Address
                                        </figcaption>
                                        <div class='ps-block__content adress-content'>
                                            <strong>

                                            </strong>
                                            <p>
                                                Address:
                                                {{ orderDetails.selectedAddress.street }},{{ orderDetails.selectedAddress.area }},{{ orderDetails.selectedAddress.city }},
                                                {{ orderDetails.selectedAddress.state }},{{ orderDetails.selectedAddress.pincode }}

                                            </p>
                                            <p>
                                                Phone : {{ orderDetails.selectedAddress.phone }}
                                            </p>
                                        </div>
                                    </figure>
                            ...
                            <div class='table-responsive'>
                                <table class='table ps-table--shopping-cart'>
                                    ...
                                    <tbody>
                                    <tr v-for='product in orderDetails.cart_items' :key='product.productId'>
                                        <td>
                                            <img
                                                width='100px'
                                        ...
                                        <td class='price'> {{ priceFormat(product.cartPrice) }}</td>
                                        <td>{{ product.liveQuantity }}</td>
                                        <td class='price'>{{ priceFormat(product.payPrice) }}</td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
                ...
                <nuxt-link to='/'>
                    <button class='ps-btn' type='button'>Back to home</button>
                </nuxt-link>
                <nuxt-link to='/account/orders'>
                    <button class='ps-btn' type='button'>Go to Orders</button>
                </nuxt-link>
            </div>
        </div>
    </section>
...
export default {
    ...
    async created() {
        ...
        this.trasnactionDetails.orderId = this.$route.query.rid;
        this.trasnactionDetails.paymentId = this.$route.query.rpid;
        if (this.trasnactionDetails.orderId.indexOf('|') === -1) {
            var products = [];
            // console.log('PARAMS', this.$route.query.rid);
            var response = await this.$http.$get(`api/orders/${this.trasnactionDetails.orderId}`);
            var data = response.data;
            this.ordersDetails.push(data);

            for (var i = 0; i < data.cart_items.length; i++) {
                products.push({
                    'name': data.cart_items[i].name,     // Name or ID is required.
                    'id': data.cart_items[i].id,
                    'price': data.cart_items[i].payPrice + '.00',
                    'brand': '',
                    'category': '',
                    'variant': '',
                    'quantity': data.cart_items[i].liveQuantity,
                    'coupon': ''
                });
            }

            // console.log('$gtm.push', JSON.stringify({
            //     'ecommerce': {
            //         'purchase': {
            //             'actionField': {
            //                 'id': this.trasnactionDetails.orderId,                         // Transaction ID. Required for purchases and refunds.
            //                 'affiliation': 'Online Store',
            //                 'revenue': data.CartSubtotal + '.00',                     // Total transaction value (incl. tax and shipping)
            //                 'tax': '0.00',
            //                 'shipping': '0.00',
            //                 'coupon': ''
            //             },
            ...
            var orderIds = this.$route.query.rid.split("|");
            for(var i =0; i < orderIds.length; i++){
                var response = await this.$http.$get(`api/orders/${orderIds[i]}`);
                var data = response.data;
                this.ordersDetails.push(data);
            }
        }
...
// http://localhost:4002/account/payment-success?rid=20220707C062AB-1%7C20220707C062AB-2&rpid=5F85C20B385B5DF8DAA75EF9
