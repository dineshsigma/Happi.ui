<template lang="html">
    <div class="ps-product ps-product--inner prd_cust" v-if="product.linker.raw != null">
        <div class="icons_box">
            <div class="wishlist-heart-lisiting">
                ...
                >
                    <i :class="wishlistedIds.includes(isWishId) == true ? 'fa fa-heart' : 'fa fa-heart-o'" 
        ...
        <a  :href="`/${product.linker.raw}`" class="product-card-hover">
        ...
        <div class="ps-product__container ">
            <div class="ps-product__content-new">
                <p class="category-head-mobile">{{ product.category.raw[0] }}</p>
                <h1 class="ps-product__title1"
                >
                    {{ product.name.raw }}
                <!-- {{ product.mrp.raw +  product.display_price.raw }} -->
            </h1>
                    ...
                    <!-- <ul>
                        <li v-if="product.stock.raw != 0">
                            <div v-if="product.display_price.raw == product.payprice.raw">
                               A <a href="">{{ priceFormat(product.display_price.raw) }}</a>
                            </div>
                            <div v-else class="ps-product__price mb-0 price-details-mobile">
                               B <a href="">{{ priceFormat(product.display_price.raw) }}</a>
                            </div> 
                        </li>
    ...
    <div class="product-bottom-inner ">
                <ul class="">
                    <li>
                        <a class="buy-now-inner"  href="#" @click.prevent="addItemToCart(product)">
                            <i class="icon-cart"></i> Add to cart
                        </a>
                    </li> &nbsp;
                    <li>
                        <a class="buy-now-inner " href="#" @click.prevent="buyNowToCart">Buy Now</a>
                    </li>
 ...
 <span><a href="/cart" class="checkout-class" style=" cursor: pointer;
    ...
    font-size: 13px;">Proceed to Checkout</a> </span>`
        ...
        async webShareApiSupported(product){
            const urlNew = 'https://www.happimobiles.com/' + (this.product.linker.raw);
            if (navigator.share) {
                navigator.share({
